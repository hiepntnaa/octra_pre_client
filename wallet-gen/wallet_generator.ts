...
async function main() {
  try {
    ...
    switch (command) {
      case "generate": {
        console.log("Ì†ΩÌ¥Ñ Generating wallet...");
...
`;
          ...
          try {
            ...
            console.log(`Ì†ΩÌ≥Å Saved to: ${filename}\n`);
            console.log(content);
          } catch (error: any) {
          ...
          console.error("‚ùå Failed to generate wallet data");
        }
        break;
      }
      ...
      case "save": {
        const data = args[1];
        if (!data) {
          console.error("‚ö†Ô∏è Missing data for save command.");
      ...
      case "derive": {
        const data = args[1];
        if (!data) {
          console.error("‚ö†Ô∏è Missing data for derive command.");
      ...
      default: {
        console.error(`‚ö†Ô∏è Unknown command: ${command}`);
        console.log("Ì†ΩÌ≥å Available commands:");
